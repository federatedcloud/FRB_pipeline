...
class Cluster:
    ...
    def DM_extrapolate(self, vchan, tchan, tstart):
        ...
        v_co = np.flip((np.arange(vchan) * params.avg_chan * params.dv) + params.freqs[0],0)
        # dispersed times, computed assuming t0 = self.t_mean, ignore tstart
        t_mean = (self.t_mean * params.dt * params.avg_samp)
        v_mean = params.freqs[-1] - (self.v_mean * params.dv * params.avg_chan)
        delayed_Ts = DM_func([self.DM * kDM], v_co, t_mean, v_mean)
        delayed_Tbins = (delayed_Ts / (params.dt * params.avg_samp)).astype(int)
        t0 = int(self.t_mean)
...
def fof(data, m1, m2, tsamp, vsamp, t_gap, v_gap, tstart):
...
def DM_extrapolate(clust, vchan, tchan, tstart):
    '''
        clust -- a cluster object
    '''
    t0 = clust.t_range[0]
    v0 = clust.v_range[0]
    DM = clust.DM
    ...
    t_seed = 0
    dT_max = 0
    for j in range(clust.N):
        if clust.v_co[j] == v_prev:
            dT_max = max((clust.t_co[j] - t_seed), dT_max)
        else:
            t_seed = clust.t_co[j]
        v_prev = clust.v_co[j]
    ...
    # disperse each frequency band, and mask
    for j in range(vchan):
        dT = (kDM * DM * ((v_co[j]**(-2.)) - (v_co[v0]**(-2.))))  # disp. delay
        dT_bins = int(dT / (params.dt * params.avg_samp)) # delay in samples
        mask[j, (t0+dT_bins - dT_width):(t0+dT_bins + dT_width)] = 20

    clust.DM_mask = mask
    return mask

def lin_extrapolate(clust, vchan, tchan, tstart):
    ...
    # disperse each frequency band, and mask
    t_co = np.arange(tchan)
    lin_v_co = lin_func(clust.linear, t_co).astype(int)
    valid_v = np.where((lin_v_co >= 0) & (lin_v_co < vchan))
    print valid_v
    print lin_v_co
    if lin_v_co[0] > lin_v_co[-1]:
    ...
    for t in range(t_min,t_max):
        mask[lin_v_co[t], (t-dT_width):(t+dT_width)] = 20
